...
- **双 Agent 路由**：中枢神经 Agent 快速判断，大脑 Agent 深度处理复杂问题
- **工具调用**：集成百度搜索、历史记录搜索、延迟发起会话
...
│   │   │   │   ├── neural.ts   # 中枢神经 Agent（快速判断，无记忆和工具）
...
机器人主逻辑（`robot.ts`）基于消息量和内容复杂度进行两级路由：
...
| `speak_to_user`       | 延迟发起会话 | 用户要求稍后提醒或定时发送消息 |
